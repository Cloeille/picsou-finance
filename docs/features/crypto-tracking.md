...
- **Cost basis is import-time market value, but editable**: a wallet/exchange holding gets `averageBuyIn = current market price` at its **first** sync (so it shows no gain/loss initially), and `upsertHolding` deliberately keeps `averageBuyIn` unchanged on every later sync. Users correct it from the holding editor (`EditHoldingModal` via `PUT /accounts/{id}/holdings/{ticker}`) by entering either the average buy-in or the **total invested** (the two are kept in sync through the quantity). For synced accounts the quantity is read-only there — it's owned by the chain/exchange and would be overwritten on the next sync — so only the cost basis changes, and the corrected value survives syncs.
